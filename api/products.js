...
export default async function handler(req, res) {
  ...
  if (req.method === 'POST') {
    try {
      ...
      // Try local fs write first (for local server)
      ...
      try {
        ...
        console.warn('fs write disabled on Vercel serverless environment:', e.message);
      }
        ...
        try {
          ...
          if (getFileRes.ok) {
            ...
            const putRes = await fetch(`https://api.github.com/repos/${GITHUB_REPO}/contents/db.json`, {
              ...
              body: JSON.stringify({
                message: 'chore: update products catalog via Admin Portal on Vercel',
