...
const LandingPage: React.FC = () => {
  ...
  const filteredProducts = products
    .filter(p => p.category === 'jewelry' || !p.category)
    .filter(p => {
      if (activeCategory === 'All') return true;
      const name = p.name.toLowerCase();
      if (activeCategory === 'Rings') return name.includes('ring') || name.includes('signet') || name.includes('band');
      if (activeCategory === 'Chains') return name.includes('chain') || name.includes('pendant') || name.includes('crux');
      if (activeCategory === 'Watches') return name.includes('chronograph') || name.includes('watch') || name.includes('obsidian');
      if (activeCategory === 'Apparel') return name.includes('kurta') || name.includes('clothing') || name.includes('silk');
      return true;
    });
...
Thank you for shopping with GemTide!`;
  ...
  return (
    <div className="relative min-h-screen bg-slate-50 text-slate-800 overflow-x-hidden selection:bg-brand-green/20 selection:text-brand-green font-sans">
      ...
      <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${
        ...
        <div className="mx-auto flex max-w-[1400px] items-center justify-between px-6 md:px-12">
          <Link to="/" className="relative z-50 flex items-center gap-2 group">
            <span className="text-2xl md:text-3xl font-extrabold font-serif tracking-[0.12em] text-slate-900 transition-colors duration-300 group-hover:text-brand-green">
              GEMTIDE
            </span>
          </Link>
      ...
      <AnimatePresence>
        {isMenuOpen && (
          <motion.div
            ...
            className="fixed inset-0 z-[45] bg-white/98 backdrop-blur-3xl flex flex-col items-center justify-center gap-8"
          >
            <a href="#collections" onClick={() => setIsMenuOpen(false)} className="text-3xl font-serif font-bold text-slate-800 hover:text-brand-green tracking-widest transition-colors duration-300">Collections</a>
            <a href="#about" onClick={() => setIsMenuOpen(false)} className="text-3xl font-serif font-bold text-slate-800 hover:text-brand-green tracking-widest transition-colors duration-300">About Us</a>
            
            {/* Expandable sub-links for More in Mobile */}
            <div className="flex flex-col items-center gap-2 border-y border-slate-100 py-3 w-full">
              <span className="text-[10px] font-tech font-bold uppercase tracking-[0.25em] text-slate-400">More</span>
              <div className="flex gap-6">
                <button 
                  onClick={() => { setIsMenuOpen(false); handleScrollToMoreTab('bags'); }} 
                  className="text-lg font-serif font-bold text-slate-700 hover:text-brand-green tracking-widest transition-colors"
                >
                  Bags
                </button>
                <button 
            ...
            <a href="#contact" onClick={() => setIsMenuOpen(false)} className="text-3xl font-serif font-bold text-slate-800 hover:text-brand-green tracking-widest transition-colors duration-300">Support</a>
            <Link
              to="/admin"
      ...
      <main className="relative z-10 mx-auto max-w-[1400px] px-6 md:px-12 pb-32">

        {/* HERO SECTION - SLEEK SILVER & JADE */}
        <section className="relative min-h-[95vh] flex flex-col lg:flex-row items-center justify-between pt-32 border-b border-slate-200/60 pb-24 gap-12">
          
          <div className="relative z-10 max-w-2xl text-left flex-shrink-0">
            <div className="inline-flex items-center gap-2 mb-6 text-brand-green font-tech text-[11px] uppercase tracking-[0.3em] font-bold bg-brand-green/10 px-4 py-2 rounded-full border border-brand-green/20">
              <Diamond size={10} className="text-brand-green animate-pulse" />
              <span>PREMIUM MODERN JEWELRY STORE</span>
            </div>

            <h1 className="font-serif text-[3.2rem] sm:text-[5rem] md:text-[6.8rem] font-extrabold text-slate-900 tracking-tight mb-8 leading-[1.05]">
              BOLD <span className="text-transparent bg-clip-text bg-gradient-to-r from-brand-green to-brand-green-accent">FORM.</span><br />
              GEN Z AESTHETIC.
            </h1>

            <p className="max-w-md text-slate-500 text-xs sm:text-sm font-sans font-light leading-relaxed tracking-wide mb-10">
            ...
            <div className="flex flex-wrap gap-6 items-center pointer-events-auto">
              <a
                href="#collections"
                className="px-8 py-3.5 rounded-full bg-brand-green hover:bg-brand-green-dark text-white text-[10px] font-bold uppercase tracking-[0.2em] transition-all duration-300 shadow-md"
              >
                Explore Collection
              </a>
              <a
                href="#contact"
                className="inline-flex items-center gap-3 text-slate-800 hover:text-brand-green transition-colors text-[10px] font-bold uppercase tracking-[0.2em]"
              >
            ...
            {/* Featured Item Card */}
            <div className="mt-12 bg-white border border-slate-200/80 p-4.5 rounded-2xl max-w-sm flex items-center gap-4.5 shadow-sm">
              <div className="h-10 w-10 rounded-xl bg-brand-green/10 flex items-center justify-center text-brand-green font-bold text-xs">
                ✦
              </div>
              <div className="flex flex-col">
                <span className="text-[10px] text-slate-400 uppercase tracking-wider font-tech">Featured Item</span>
                <span className="text-sm font-bold text-slate-800 tracking-wide font-sans mt-0.5">Aurelia Serpent Band</span>
              </div>
            </div>
          </div>

          {/* Right Column: 3D rotating gemstone viewport + Floating Tags */}
          <div className="relative w-full lg:w-[500px] h-[400px] md:h-[500px] flex items-center justify-center flex-grow">
            
            {/* Ambient halo behind 3D Canvas */}
            <div className="absolute w-[300px] h-[300px] md:w-[460px] md:h-[460px] rounded-full border border-slate-200 pointer-events-none flex items-center justify-center z-0">
            ...
            <div className="absolute inset-0 flex flex-col justify-between p-4 pointer-events-none z-10 font-tech">
              <div className="flex justify-between items-start">
                <div className="bg-white/90 backdrop-blur-md border border-slate-200 px-4 py-2 rounded-full text-center shadow-xs">
                  <span className="text-[9px] font-bold text-brand-green uppercase tracking-widest block">✦ 100% Aesthetic Guarantee</span>
                </div>
                <div className="bg-white/90 backdrop-blur-md border border-slate-200 px-4 py-2 rounded-full text-center shadow-xs">
                  <span className="text-[9px] font-bold text-slate-800 uppercase tracking-widest block">5000+ Happy Customers</span>
                </div>
              </div>
              <div className="flex justify-between items-end">
                <div className="bg-white/90 backdrop-blur-md border border-slate-200 px-4 py-2 rounded-full text-center shadow-xs">
                  <span className="text-[9px] font-bold text-brand-green uppercase tracking-widest block">✦ Premium Alloy Finish</span>
                </div>
                {/* Visual Auction Countdown timer */}
                <div className="bg-white/95 backdrop-blur-md border border-slate-200 p-3 rounded-2xl flex gap-3 text-center shadow-sm">
                  <div className="flex flex-col">
        ...
        <div className="w-[100vw] relative left-1/2 -translate-x-1/2 border-y border-slate-200 bg-white/60 backdrop-blur-md overflow-hidden py-8 my-20">
          <div className="animate-marquee whitespace-nowrap flex gap-20 items-center font-tech font-bold text-[10px] text-slate-600 uppercase tracking-[0.3em]">
        ...
        {/* COLLECTIONS GRID */}
        <section id="collections" className="py-20 scroll-mt-24">
          <div className="mb-20 flex flex-col xl:flex-row justify-between xl:items-end gap-8 pb-10 border-b border-slate-200">
            <div>
              <span className="text-[10px] font-tech uppercase tracking-[0.25em] text-brand-green font-bold bg-brand-green/10 px-3 py-1.5 rounded-full border border-brand-green/20">LIMITED RELEASES</span>
              <h2 className="font-serif text-4xl sm:text-5xl md:text-6xl text-slate-900 font-bold mt-4">Signature Catalog</h2>
            </div>
            
            {/* Category Filter Bar - Rounded Pills */}
            <div className="flex flex-wrap gap-2.5">
              {categories.map(cat => (
                <button
                  key={cat}
                  onClick={() => setActiveCategory(cat)}
                  className={`relative px-6 py-3 text-[10px] font-bold uppercase tracking-[0.2em] transition-all duration-350 rounded-full border ${
                    activeCategory === cat
              ...
              {filteredProducts.map(product => {
                ...
                return (
                  <motion.div
                    ...
                    className="group relative flex flex-col bg-white rounded-3xl border border-slate-100 hover:border-slate-200 shadow-sm hover:shadow-md transition-all duration-300 overflow-hidden"
                  >
                    {/* Image Box */}
                    <div className="relative aspect-[3/4] overflow-hidden bg-slate-50 m-3 rounded-2xl border border-slate-100">
                      <img
                        src={product.image}
                        alt={product.name}
                      ...
                      <button
                        ...
                        className={`absolute top-5 right-5 p-2.5 rounded-full backdrop-blur-md border transition-all duration-300 z-20 ${
                          isFav 
                      ...
                      >
                        <Heart size={12} fill={isFav ? 'currentColor' : 'none'} strokeWidth={isFav ? 0 : 2} />
                      </button>

                      {/* Sold Out Overlay */}
                      {isOutOfStock && (
                        <div className="absolute inset-0 bg-white/80 backdrop-blur-xs flex items-center justify-center z-10">
                          <span className="text-[9px] tracking-[0.3em] font-tech uppercase font-bold text-slate-800 border border-slate-200 px-5 py-2.5 bg-white/95 rounded-full shadow-xs">
                            OUT OF STOCK
                          </span>
                        </div>
                      )}
                      ...
                      {product.offer > 0 && !isOutOfStock && (
                        <div className="absolute bottom-5 left-5 bg-white/90 backdrop-blur-md text-brand-green text-[9px] font-tech font-bold uppercase tracking-[0.15em] px-3.5 py-1.5 border border-slate-200 rounded-full shadow-xs">
                          {product.offer}% OFF
                        </div>
                      )}
                    </div>
                    ...
                    <div className="p-7 flex flex-col flex-grow">
                      <div className="flex justify-between items-center text-[9px] font-tech text-slate-400 uppercase tracking-[0.2em] font-semibold">
                        <span>{product.tag}</span>
                        {product.stock <= 4 && product.stock > 0 && (
                          <span className="text-brand-green tracking-widest font-bold">ONLY {product.stock} LEFT</span>
                        )}
                      </div>

                      <h3 className="font-serif text-2xl text-slate-900 font-bold mt-3 group-hover:text-brand-green transition-colors duration-300">
                        {product.name}
                      </h3>
                      ...
                      <div className="mt-5 flex items-baseline gap-3">
                        ...
                        {product.offer > 0 && (
                          <span className="text-xs text-slate-400 line-through tracking-wider">
                      ...
                      <div className="mt-8 pt-6 border-t border-slate-100">
                        <button
                          ...
                          className={`w-full py-3.5 text-center text-[10px] font-bold uppercase tracking-[0.25em] transition-all duration-300 rounded-full border ${
                            isOutOfStock
        ...
        <section id="more" className="py-20 border-t border-slate-200 scroll-mt-24">
          <div className="mb-16 flex flex-col xl:flex-row justify-between xl:items-end gap-8 pb-10 border-b border-slate-200">
            <div>
              <span className="text-[10px] font-tech uppercase tracking-[0.25em] text-brand-green font-bold bg-brand-green/10 px-3 py-1.5 rounded-full border border-brand-green/20">LIFESTYLE EXTENSIONS</span>
              <h2 className="font-serif text-4xl sm:text-5xl md:text-6xl text-slate-900 font-bold mt-4">More than Ornaments</h2>
            </div>
          ...
          {products.filter(p => p.category === activeMoreTab).length === 0 ? (
            <div className="text-center py-28 border border-slate-200 bg-white rounded-3xl flex flex-col items-center justify-center shadow-xs">
              <Sparkles className="text-brand-green/20 mb-4 animate-spin" style={{ animationDuration: '10s' }} size={32} />
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10 lg:gap-12">
              {products.filter(p => p.category === activeMoreTab).map(product => {
                const isOutOfStock = product.stock <= 0;
                const finalPrice = effectivePrice(product.price, product.offer);
                const isFav = favorites.includes(product.id);
        ...
        {/* HERITAGE SECTION */}
        <section id="about" className="py-32 border-t border-slate-200 my-16">
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-16 items-center">
            
            <div className="lg:col-span-5 space-y-8">
              <span className="text-[10px] font-tech uppercase tracking-[0.3em] text-brand-green font-bold bg-brand-green/10 px-3 py-1.5 rounded-full border border-brand-green/20">ABOUT GEMTIDE</span>
              <h2 className="font-serif text-4xl sm:text-5xl text-slate-900 font-bold leading-tight">Modern Jewelry,<br />Crafted for Daily Wear.</h2>
              ...
              <div className="grid grid-cols-3 gap-4 pt-10 border-t border-slate-200">
                <div className="space-y-1">
                  <Award className="text-brand-green mb-2 animate-pulse" size={20} strokeWidth={1.5} />
                  <h4 className="text-[10px] font-tech font-bold text-slate-800 uppercase tracking-wider">Modern Design</h4>
                  <p className="text-[9px] text-slate-400 font-light">Premium Alloys</p>
                </div>
                <div className="space-y-1">
                  <Truck className="text-slate-600 mb-2" size={20} strokeWidth={1.5} />
                  <h4 className="text-[10px] font-tech font-bold text-slate-800 uppercase tracking-wider">Free Shipping</h4>
                  <p className="text-[9px] text-slate-400 font-light">Pan-India</p>
                </div>
                <div className="space-y-1">
                  <ShieldCheck className="text-brand-green mb-2" size={20} strokeWidth={1.5} />
                  <h4 className="text-[10px] font-tech font-bold text-slate-800 uppercase tracking-wider">Durability</h4>
                  <p className="text-[9px] text-slate-400 font-light">Anti-Tarnish Finish</p>
                </div>
              </div>
            </div>
            ...
            <div className="lg:col-span-7 relative flex justify-end">
              {/* Decorative Frame */}
              <div className="absolute top-[-20px] left-[-20px] w-[50px] h-[50px] border-t border-l border-slate-300 pointer-events-none" />
              <div className="absolute bottom-[-20px] right-[-20px] w-[50px] h-[50px] border-b border-r border-slate-300 pointer-events-none" />

              <div className="relative w-[90%] aspect-[4/3] sm:aspect-video lg:aspect-[4/3] overflow-hidden bg-slate-100 border border-slate-200 rounded-3xl group shadow-sm">
                <img
                  src="https://images.unsplash.com/photo-1573408301185-9146fe634ad0?auto=format&fit=crop&q=80&w=1200"
                  alt="Jaipur Craft Atelier"
                ...
                />
                <div className="absolute inset-0 bg-gradient-to-t from-slate-900/90 via-slate-900/10 to-transparent flex items-end p-8">
                  <div>
                    <span className="text-[8px] font-tech tracking-[0.2em] uppercase text-brand-green font-bold">OUR DESIGN</span>
                    <p className="font-serif text-lg text-white font-bold mt-1">Sleek Ready-to-Wear Jewelry</p>
                  </div>
                </div>
              </div>
            </div>
        ...
        <section id="contact" className="py-20 border-t border-slate-200">
          <div className="relative bg-white border border-slate-200 rounded-3xl p-10 md:p-20 text-center max-w-4xl mx-auto shadow-sm">
            ...
            <h2 className="font-serif text-3xl sm:text-4xl md:text-5xl text-slate-900 font-bold mt-3 mb-5">Subscribe to our Newsletter</h2>
            <p className="text-slate-500 text-xs sm:text-sm max-w-md mx-auto mb-10 font-light tracking-wide leading-relaxed">
            ...
            <form className="flex flex-col sm:flex-row gap-4 max-w-lg mx-auto" onSubmit={(e) => { e.preventDefault(); alert('Subscribed successfully!'); }}>
              <input
                ...
                className="flex-grow bg-slate-50 border border-slate-200 rounded-full px-6 py-4 text-xs text-slate-800 placeholder-slate-400 focus:outline-none focus:border-brand-green transition-colors"
              />
              <button
                type="submit"
                className="bg-brand-green hover:bg-brand-green-dark text-white font-bold text-[9px] uppercase tracking-[0.25em] px-8 py-4 rounded-full transition-all duration-300 shadow-xs"
              >
                Subscribe
              </button>
            </form>
      ...
      {/* FOOTER */}
      <footer className="bg-slate-100 border-t border-slate-200 py-16 relative z-10">
        <div className="mx-auto max-w-[1400px] px-6 md:px-12">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12 pb-16">
            ...
            <div className="space-y-4">
              ...
              <div className="flex flex-col gap-2.5 text-[9px] uppercase tracking-wider text-slate-500 font-tech font-bold">
        ...
        {isCartOpen && (
          <>
            ...
            <motion.div
              ...
              className="fixed right-0 top-0 bottom-0 z-50 w-full max-w-[420px] bg-white border-l border-slate-200 flex flex-col shadow-xl"
            >
              {/* Header */}
              ...
              <div className="flex-grow overflow-y-auto p-6 space-y-6">
                {cart.length === 0 ? (
                  ...
                  cart.map(item => {
                    const finalPrice = effectivePrice(item.product.price, item.product.offer);
                    return (
                      <div key={item.product.id} className="flex gap-4 pb-6 border-b border-slate-100 group">
                        {/* Image */}
              ...
              {cart.length > 0 && (
                <div className="p-6 border-t border-slate-100 bg-slate-50 space-y-5">
                  ...
                  <button
                    ...
                    className="w-full bg-brand-green hover:bg-brand-green-dark text-white rounded-full py-4 text-center text-[10px] font-bold uppercase tracking-[0.25em] transition-all shadow-md"
                  >
                    Proceed to Checkout
                  </button>
                </div>
        ...
        {isCheckoutOpen && (
          <div className="fixed inset-0 z-[55] flex items-center justify-center p-4">
            ...
            <motion.div
              ...
              className="relative w-full max-w-[480px] bg-white border border-slate-200 rounded-3xl p-6 sm:p-8 shadow-xl z-[55] text-left"
            >
              <button 
                onClick={() => setIsCheckoutOpen(false)}
              ...
              <div className="mb-8 flex items-center gap-2">
                ...
                <h3 className="font-serif text-2xl font-bold text-slate-800 uppercase tracking-widest">Order Details</h3>
              </div>

              <form onSubmit={handleCheckoutSubmit} className="space-y-5 text-[10px] font-tech tracking-wider uppercase font-bold text-slate-500">
                <div>
                  ...
                  <input
                    ...
                    className="w-full bg-slate-50 border border-slate-200 rounded-full px-5 py-3.5 text-slate-800 placeholder-slate-400 focus:outline-none focus:border-brand-green transition-colors text-xs font-normal tracking-wide normal-case"
                  />
                </div>
                ...
                <div>
                  ...
                  <textarea
                    ...
                    className="w-full bg-slate-50 border border-slate-200 rounded-2xl px-5 py-3.5 text-slate-800 placeholder-slate-400 focus:outline-none focus:border-brand-green transition-colors text-xs font-normal tracking-wide normal-case resize-none"
                  />
                </div>
                ...
                <div>
                  ...
                  <div className="grid grid-cols-2 gap-3 text-[9px]">
                    <button
                      ...
                      className={`p-3.5 rounded-full border text-center transition-all ${
                ...
                <div className="pt-6 border-t border-slate-100 space-y-2 text-[10px] font-tech uppercase tracking-widest text-slate-500">
                  ...
                  <div className="flex justify-between items-center text-slate-800 pt-2 border-t border-dashed border-slate-200">
                    ...
                    <span className="text-brand-green text-lg font-bold">{formatPrice(grandTotal)}</span>
                  </div>
                </div>

                <button
                  type="submit"
                  className="w-full bg-brand-green hover:bg-brand-green-dark text-white rounded-full py-4.5 text-center text-[10px] font-bold uppercase tracking-[0.25em] transition-all shadow-md mt-8"
                >
        ...
        {orderSuccess && (
          <div className="fixed inset-0 z-[55] flex items-center justify-center p-4">
            ...
            <motion.div
              ...
              animate={{ opacity: 1, scale: 1 }}
              exit={{ opacity: 0, scale: 0.97 }}
              className="relative w-full max-w-[420px] bg-white border border-slate-200 rounded-3xl p-8 text-center shadow-xl z-[55]"
            >
              <div className="w-14 h-14 rounded-full bg-brand-green/10 border border-brand-green/20 mx-auto flex items-center justify-center text-brand-green mb-6 animate-pulse">
                <Check size={24} />
              </div>
              <h3 className="font-serif text-2xl text-slate-900 font-bold uppercase tracking-widest mb-3">Order Details Ready</h3>
              <p className="text-slate-500 text-[10px] leading-relaxed tracking-wider font-light mb-8 normal-case">
              ...
              <button
                onClick={() => setOrderSuccess(false)}
                className="w-full bg-brand-green hover:bg-brand-green-dark text-white py-4 text-[9px] font-bold uppercase tracking-[0.25em] rounded-full transition-colors duration-300 shadow-md"
              >
                Continue Shopping
              </button>
            </motion.div>
